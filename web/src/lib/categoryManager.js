...
export async function addCustomCategory(category) {
  const custom = await getCustomCategories();
  const id = category.id || `custom_${category.name.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '')}`;
...
export async function updateCustomCategory(id, changes) {
  const custom = await getCustomCategories();
...
export async function toggleCategoryVisibility(categoryId) {
  const hidden = await getHiddenCategories();
  let updated;
...
export function isCustomCategory(id) {
  return id?.startsWith('custom_') || false;
}
...
async function _saveCustomCategories(categories) {
  ...
  try { await settingsApi.set(CUSTOM_CATEGORIES_KEY, categories); } catch {}
}

async function _saveHiddenCategories(hidden) {
  _hiddenCategories = hidden;
  await setSetting(HIDDEN_CATEGORIES_KEY, hidden);
  try { await settingsApi.set(HIDDEN_CATEGORIES_KEY, hidden); } catch {}
}
