...
export default function NetWorth() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setForm({ name: '', type: 'checking', balance: '', currency, icon: '🏦', color: '#1B7A6E' });
      loadAccounts();
  ...
  const handleEdit = (acct) => {
    setEditAccount(acct);
    setForm({ name: acct.name, type: acct.type, balance: acct.balance.toString(), currency: acct.currency || currency, icon: acct.icon, color: acct.color || '#1B7A6E' });
    setShowForm(true);
  };
    ...
    <div className="card group">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg" style={{ backgroundColor: (account.color || '#1B7A6E') + '15' }}>
            {account.icon || '🏦'}
          </div>
          <div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setEditAccount(null); setForm({ name: '', type: 'checking', balance: '', currency, icon: '🏦', color: '#1B7A6E' }); setShowForm(true); }} className="btn-primary text-xs flex items-center gap-1"><Plus size={14} /> {t('networth.addAccount')}</button>
      </div>
